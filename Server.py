...
from utils import generate_text_table_html, check_exchange_message, handle_exchange, get_data
    ...
    async def distribute(self, ws: WebSocketServerProtocol):
        async for message in ws:
            ...
            if is_exchange:
                ...
                await handle_exchange(f'{ws.name}: {message}; Request handled for {rates_days} days and {currencies}')

                await self.send_to_clients(
                    f'Server: getting rates from Privat Bank for {rates_days} days and {currencies}')
